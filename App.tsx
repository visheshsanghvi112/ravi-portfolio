...
import { Instagram, Mail, Phone, Menu, X, ArrowRight, Camera, Star, ChevronLeft, ChevronRight, Maximize2, Upload } from 'lucide-react';
import { Button, SectionTitle } from './components/Shared';
import { AIStudio } from './components/AIStudio';
import { ChatBot } from './components/ChatBot';
import { MediaUploader, getStoredMedia } from './components/MediaUploader';
...
function App() {
  ...
  // Load and merge user-uploaded media with placeholder images
  useEffect(() => {
    const loadMedia = () => {
      const userMedia = getStoredMedia();
      const userImages = userMedia
        .filter(m => m.type === 'image')
        .map(m => m.url);
      setAllImages([...userImages, ...PLACEHOLDER_IMAGES]);
    };

    loadMedia();
    // Listen for storage changes (when media is uploaded)
    window.addEventListener('storage', loadMedia);
    // Also listen for custom event we'll dispatch
    window.addEventListener('mediaUpdated', loadMedia);

    return () => {
      window.removeEventListener('storage', loadMedia);
      window.removeEventListener('mediaUpdated', loadMedia);
    };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-obsidian text-gray-200 font-sans overflow-x-hidden selection:bg-gold-400 selection:text-black">
      ...
      {/* --- Admin Panel --- */}
      {showAdmin && (
        <section className="min-h-screen py-32 bg-charcoal/50 relative">
          <div className="container mx-auto px-6">
            <div className="max-w-6xl mx-auto">
              <div className="flex items-center justify-between mb-12">
                <div>
                  <h2 className="text-4xl font-serif text-white mb-2">Admin Panel</h2>
                  <p className="text-gray-400">Manage your portfolio media</p>
                </div>
                <button
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
          {allImages.length > 0 ? (
            ...
            <div className="col-span-full text-center py-20">
              ...
              <p className="text-gray-500 text-sm mb-6">Upload your images via the admin panel</p>
              <Button
                variant="outline"
